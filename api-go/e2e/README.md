...
E2E-тесты проверяют HTTP и S3-эндпоинты `api-go` через реальный клиент на `aiobotocore` и через MinIO Client `mc` для узкого live smoke по S3-compatible API.
...
- MinIO Client `mc alias set --path on`, `mc cp`, `mc ls`, `mc cat`, `mc rm` against `/api/s3`
...
Pipeline `.woodpecker/api-go.yml` запускает `docker-compose.e2e.yml` через DinD только для `s3`. Этот режим использует локальный MinIO из `docker-compose.e2e.yml`, добавляет pinned `mc` binary в e2e-образ и является обязательным PR-gate.
